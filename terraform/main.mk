...
CHECKOV ?= $(DOCKER) run --user "$(shell id -u):$(shell id -g)" -v $(ENV_DIR):/tf -i bridgecrew/checkov -d /tf -s
TFLINT ?= $(DOCKER) run --user "$(shell id -u):$(shell id -g)" --rm -v $(ENV_DIR):/data -t wata727/tflint
TFLOCK ?= $(DOCKER) run --user "$(shell id -u):$(shell id -g)" --rm --hostname=$(USER)-icmk-terraform -v $(ENV_DIR):/$(ENV_DIR) -v "$(ENV_DIR)/.terraform":/"$(ENV_DIR)/.terraform" -v "$(INFRA_DIR)":"$(INFRA_DIR)" -v $(HOME)/.aws/:/.aws:ro -w $(ENV_DIR) -e AWS_PROFILE=$(AWS_PROFILE) -e ENV=$(ENV) hazelops/tflock
TF_LOG_LEVEL ?= 
TF_LOG_PATH ?= /$(ENV_DIR)/tflog.txt
TERRAFORM ?= $(DOCKER) run --user "$(shell id -u):$(shell id -g)" --rm --hostname=$(USER)-icmk-terraform -v $(ENV_DIR):/$(ENV_DIR) -v "$(ENV_DIR)/.terraform":/"$(ENV_DIR)/.terraform" -v "$(INFRA_DIR)":"$(INFRA_DIR)" -v $(HOME)/.aws/:/.aws:ro -w $(ENV_DIR) -e AWS_PROFILE=$(AWS_PROFILE) -e ENV=$(ENV) -e TF_LOG=$(TF_LOG_LEVEL) -e TF_LOG_PATH=$(TF_LOG_PATH) hashicorp/terraform:$(TERRAFORM_VERSION)
