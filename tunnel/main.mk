...
BASTION_INSTANCE_ID = $(shell $(AWS) ssm get-parameter --name "/$(ENV)/terraform-output" --with-decryption | $(JQ) -r '.Parameter.Value' | $(BASE64) -d | $(JQ) -r '.bastion_instance_id.value')
...
CMD_BASTION_SSH_TUNNEL_UP = $(shell $(AWS) ssm get-parameter --name "/$(ENV)/terraform-output" --with-decryption | $(JQ) -r '.Parameter.Value' | $(BASE64) -d | $(JQ) -r '.cmd.value.tunnel.up') -F $(SSH_CONFIG) && echo "\n\033[32m[OK]\033[0m SSH tunnel up"
CMD_BASTION_SSH_TUNNEL_DOWN = $(shell $(AWS) ssm get-parameter --name "/$(ENV)/terraform-output" --with-decryption | $(JQ) -r '.Parameter.Value' | $(BASE64) -d | $(JQ) -r '.cmd.value.tunnel.down') -F $(SSH_CONFIG) && echo "\n\033[32m[OK]\033[0m SSH tunnel disabled"
CMD_BASTION_SSH_TUNNEL_STATUS = $(shell $(AWS) ssm get-parameter --name "/$(ENV)/terraform-output" --with-decryption | $(JQ) -r '.Parameter.Value' | $(BASE64) -d | $(JQ) -r '.cmd.value.tunnel.status') -F $(SSH_CONFIG) && echo "\n\033[32m[OK]\033[0m SSH tunnel up.\nConfig:\n-----" && cat $(SSH_CONFIG)
...
# Tasks
########################################################################################################################
tunnel: tunnel.config tunnel.up
tunnel.up:
