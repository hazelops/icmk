...
ECS_SERVICE_TASK_ID_TEMPFILE = $(INFRA_DIR)/ecs_task_id
ECS_SERVICE_TASK_ID = $(shell $(AWS) ecs run-task --cluster $(ECS_CLUSTER_NAME) --task-definition "$(ECS_SERVICE_TASK_DEFINITION_ARN)" --network-configuration '$(ECS_SERVICE_TASK_NETWORK_CONFIG)' --launch-type "$(ECS_SERVICE_TASK_LAUNCH_TYPE)" | $(JQ) -r '.tasks[].taskArn' | $(REV) | $(CUT) -d'/' -f1 | $(REV) > $(ECS_SERVICE_TASK_ID_TEMPFILE) && cat $(ECS_SERVICE_TASK_ID_TEMPFILE) && sleep 1)
...
CMD_ECS_SERVICE_TASK_ID = $$(cat "$(ECS_SERVICE_TASK_ID_TEMPFILE)")
CMD_ECS_SERVICE_TASK_LOG = $(ECS_CLI) logs --task-id "$(CMD_ECS_SERVICE_TASK_ID)" --cluster "$(ECS_CLUSTER_NAME)"
CMD_ECS_SERVICE_TASK_GET_LOG = until echo $$($(CMD_ECS_SERVICE_TASK_LOG)) | grep -Fqe "["; do sleep 2; done
