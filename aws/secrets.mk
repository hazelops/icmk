...
CMD_SERVICE_SECRETS_PUSH = @ (echo $(foreach item, $(SERVICE_SECRETS), \
		$(shell $(AWS) ssm put-parameter --name="/$(ENV)/$(SVC)/$(item)" --value="$(shell \
...
CMD_SERVICE_SECRETS_DELETE = @ (echo $(foreach item, $(shell $(AWS) ssm get-parameters-by-path \
		--path "/$(ENV)/$(SVC)" --query "Parameters[*].Name" --recursive | $(JQ) -e -r '. | to_entries[] | .value' ), \
		$(shell $(AWS) ssm delete-parameter --name $(item))) > /dev/null ) && \
		echo "\033[32m[OK]\033[0m /$(ENV)/$(SVC)/* secrets deleted" || echo "\033[31m[ERROR]\033[0m /$(ENV)/$(SVC)/* secrets deletion"

CMD_SERVICE_ALL_SECRET_KEYS = $(foreach item, $(shell $(AWS) ssm get-parameters-by-path \
...
CMD_SERVICE_SECRETS_PULL = @ (echo $(shell echo "{\"INFO\":\"EMPTY_JSON_CREATED\"}" > $(SERVICE_SECRETS_BACKUP_FILE)) && \
		...
		$(shell $(JQ) --arg value "$(shell echo $(item) | sed 's|.*/||')" '.[$$value] = "'$(shell $(AWS) ssm get-parameter --with-decryption --name $(item) --query Parameter.Value)'"' \
