...
########################################################################################################################
# SSM Wrapper
SSM_DOCKER_TAG ?= hazelops/ssm-wrapper:latest
SSM ?= $(DOCKER) run -u nobody -v $(HOME)/.aws/:/root/.aws -v $(ENV_DIR):/$(ENV_DIR) -e AWS_PROFILE=$(AWS_PROFILE) -e AWS_REGION=$(AWS_REGION) $(SSM_DOCKER_TAG) ssm
